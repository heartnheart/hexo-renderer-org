...
function print_warning(err, path) {
  ...
  var useless = [
    ...
    'Cannot fontify src block (htmlize.el >= 1.34 required)', // for Windows
    ''
  ];
  var lines = err.split('\n');
  var msg = '';
  ...
  if (msg != '')
    process.stderr.write(`XXX==Msg Here==>${path}:\n${msg}`);
}
...
(progn
  ...
  `;
    var exec_args = ['--batch', '--kill', '--execute', emacs_lisp];
